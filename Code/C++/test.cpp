...
DB U(int j,int i,Pos posk,Pos pos){
    ...
    if(i==0||i==2||i==3||i==4||i==5) return 0;
    if(i==6){
        if(j==0) return 0.004*365;
        if(j==4) return 0.0023*365;
        return 0;
    }
    if(i==7) return 1.0/(dis(posk,pos)+1.0);
    if(i==1) return -1.0/(dis(posk,pos)+1.0);
}
DB suit[10][10]={
    {7,1.43,0,0,6},
    {0.35,-1,-1,-1,0.3},
...
DB V(int i,int j,DB a){
    ...
    DB ssuit=suit[i][j];
    if(ssuit==-1) return 0.5;
    if(ssuit==2e9) return a/1000;
    return 1/(abs(a-ssuit)+1);
}
DB a0[60][40][10]={};
void init(){
    freopen("C:/Users/linru/Desktop/IMMC/Code/Python/out","r",stdin);
    for(int x=0;x<xmax;x++) for(int y=0;y<ymax;y++) cin>>a0[x][y][0];
    freopen("C:/Users/linru/Desktop/IMMC/Code/Python/tree","r",stdin);
    for(int x=0;x<xmax;x++) for(int y=0;y<ymax;y++) cin>>a0[x][y][1]; 
    freopen("pos.txt","r",stdin);
    int x,y;
    while(cin>>x>>y){
        pos[N++]=Pos(x,y);
        if(N<=120) type[N]=0;
        else if(N<=208) type[N]=1;
        else type[N]=0;
    }
}
void dif(DB a0[][40][10],DB W[]){
    DB sumW=0;
    for(int i=0;i<M;i++) sumW+=W[i];
    for(int x=0;x<xmax;x++) for(int y=0;y<ymax;y++) for(int i=0;i<M;i++) a[x][y][i][0]=a0[x][y][i];
    memset(A,0,sizeof(A));
    for(int t=1;t<T;t++){
        for(int k=0;k<N;k++){
            for(int i=0;i<M;i++) A[k][t]+=V(i,type[k],a[pos[k].x][pos[k].y][i][t-1])*W[i];
            A[k][t]/=sumW;
        }
        for(int kk=0;kk<N;kk++){
            ...
            for(int i=1;i<M;i++){
                DB da=0;
                for(int k=0;k<N;k++) da+=A[k][t]*U(type[k],i,pos[k],pos[kk]);
...
DB S(int j,DB tim){return tim?s[j]:-c[j];}
DB h(int flag,DB tim){return (-2.0*flag+2.0)+(-2.0*flag+3.0)*((DB)1.0)/(exp(tim-1.0)+1.0);}
DB rate(DB r,DB tim){return r*h(1,tim)+(1-r)*h(2,tim);}
DB P(DB r,DB a0[][40][10],DB W[]){
    ...
    for(int t=0;t<T;t++){
        DB tim=((DB)t)*dt;
        DB dp=0;
        for(int k=0;k<N;k++) dp+=t?(A[k][t]*S(type[k],tim)*rate(r,tim)):(S(type[k],tim)*rate(r,tim));
        p+=dp*dt;
    }
    return p;
}
