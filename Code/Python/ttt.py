...
#     ws = wb.add_sheet(str(ty))
#     typ=[ty]
#     for x in range(maxx):
#         for y in range(maxy):
#             pos=[Pos(x,y)]
#             ws.write(x, y, P(0.2,2,a0,W))

# wb.save('k.xls')
# for i in range(10):
#     W=W+np.array([0,-0.1,0,0,0,0,0.1,0.1])
#     print(P(0.2,10,a0,W))
...
for x in range(maxx):
    ...
    for y in range(maxy):
        ans=[]
        for ty in range(5):
            pos=np.array([Pos(x,y)])
            typ=[ty]
            N=1
            ans.append({'type':ty,'num':P(0.2,2,a0,W)})
        ans.sort(reverse=True,key=rule)
        out[x].append(ans[0].get('type'))
        ws.write(x, y, out[x][y])
